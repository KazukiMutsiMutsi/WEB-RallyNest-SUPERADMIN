...
export default function AdminBookings() {
  const { bookings, courts } = useTenant();
  const [search,  setSearch]  = useState('');
  const [statusF, setStatusF] = useState('all');
  const [courtF,  setCourtF]  = useState('all');

  const filtered = (bookings as AdminBooking[]).filter(b => {
    if (statusF !== 'all' && b.status !== statusF) return false;
    if (courtF  !== 'all' && b.courtId !== courtF) return false;
    if (search.trim()) {
      const q = search.toLowerCase();
      return b.playerName.toLowerCase().includes(q) || b.id.toLowerCase().includes(q);
    }
    return true;
                ...
                <tr key={b.id} style={{ ...s.tr, background: i%2===0 ? '#fff' : '#fafafa' }}>
                  ...
                  <td style={s.td}>
                    <div style={{ display:'flex', alignItems:'center', gap:9 }}>
                      ...
                      <div><div style={{ fontWeight:700, fontSize:13 }}>{b.playerName}</div><div style={{ fontSize:11, color:'#94a3b8' }}>{b.playerPhone}</div></div>
                    </div>
                  </td>
...
const s: Record<string, React.CSSProperties> = {
  ...
  select:      { height:40, padding:'0 12px', border:'1.5px solid #e2e8f0', borderRadius:9, fontSize:13, background:'#fff', outline:'none', cursor:'pointer' },
