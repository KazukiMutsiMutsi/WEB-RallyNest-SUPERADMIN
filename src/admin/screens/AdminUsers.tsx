    ...
    <div style={s.page}>
      ...
      <div style={s.strip}>
        {(['all', 'active', 'flagged', 'banned'] as const).map(st => (
          <button key={st} onClick={() => setStatusF(st as any)}
...
function ActionBtn({ label, color, bg, border, onClick }: { label:string; color:string; bg:string; border:string; onClick:()=>void }) {
  return (
    <button onClick={onClick}
      style={{ padding:'4px 10px', borderRadius:6, border:`1px solid ${border}`, background:bg, color, fontSize:11, fontWeight:700, cursor:'pointer' }}>
      {label}
    </button>
  );
}
